...
int n,m,s,t,p;
vector<int>vis(maxn),dist1(maxn),dist2(maxn);
vector<pair<int,int> > edge[maxn],redge[maxn];
///solution
void dijkstra(int x){
    dist1[x]=0;
...
void solution(){
    ...
    s--,t--;
    vector<tuple<int,int,int> > kahini;
    for(int i=0;i<m;i++){
        int u,v,w;
        scanf("%d%d%d",&u,&v,&w);
        u--,v--;
    ...
    for(auto it:kahini){
        ...
        //debug(dist1[u],dist2[v]);
        if(dist1[u]==highest(int) || dist2[v]==highest(int))continue;
        int mot=dist1[u]+w+dist2[v];
        if(mot<=p){
            ans=max(ans,w);
        }
    }
