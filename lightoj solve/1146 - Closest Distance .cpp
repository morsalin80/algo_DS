...
double eu_d(point a, point b){
    double xx=(a.x-b.x)*(a.x-b.x) + (a.y-b.y)*(a.y-b.y);
    return sqrt(xx);
}
int cs=0;
void solution(){
    ...
    double ac= eu_d(a,c);
    double bd= eu_d(b,d);
    double ans=highest(double);
    ans=min(ac,bd);
    while(abs(ac-bd)>0.000001){
        point tempa,tempb,tempc,tempd;
        ///(2*l +r)/3
        tempa.x=(2*a.x + b.x)/3.0;
        tempa.y=(2*a.y + b.y)/3.0;
        tempb.x=(2*b.x + a.x)/3.0;
        tempb.y=(2*b.y + a.y)/3.0;
        tempc.x=(2*c.x + d.x)/3.0;
        tempc.y=(2*c.y + d.y)/3.0;
        tempd.x=(2*d.x + c.x)/3.0;
        tempd.y=(2*d.y + c.y)/3.0;
        ac=eu_d(tempa,tempc);
        bd=eu_d(tempb,tempd);
        ans=min(ans,min(ac,bd));
        if(ac>bd)a=tempa,c=tempc;
        else b=tempb,d=tempd;
    }
