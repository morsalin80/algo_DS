...
struct sparse{
    ...
    void pre(){
        ...
        for(int j=1;j<=nc;j++){
            for(int i=0;i+po(2,j)-1<maxn;i++){
                if(temp[tt[i][j-1]]>temp[tt[i+po(2,j-1)][j-1]]){
                    tt[i][j]=tt[i][j-1];
                }
                else{
                    tt[i][j]=tt[i+po(2,j-1)][j-1];
                }
            }
        }
    }
    int minq(int l,int r){
        int mot=r-l+1;
        int k=log2(mot);
        int slide=l+(mot-po(2,k));
        return max(temp[tt[l][k]],temp[tt[slide][k]]);
    }
};
int cs=0;
void solution(){
    ...
    for(int l=0;l<q;l++){
        ...
        int is=i+s-1,js=j+s-1;
        i--,j--,is--,js--;
        int ans=lowest(int);
        for(int m=i;m<=is;m++){
            ans=max(ans,table[m].minq(j,js));
        }
        printf("%lld\n",ans);
    }
}
