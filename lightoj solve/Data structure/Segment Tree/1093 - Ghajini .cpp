...
int maxtree[3*maxn];
int mintree[3*maxn];
void max_build(int node,int l,int r){
    if(l==r){
        maxtree[node]=arr[l];
        return;
    }
    ...
    int right=(2*node)+1;
    int mid=(l+r)/2;
    max_build(left,l,mid);
    max_build(right,mid+1,r);
    maxtree[node]=max(maxtree[left],maxtree[right]);
}
void min_build(int node,int l, int r){
    if(l==r){
        mintree[node]=arr[l];
        return;
    }
    ...
    int right=2*node+1;
    int mid=(l+r)/2;
    min_build(left,l,mid);
    min_build(right,mid+1,r);
    mintree[node]=min(mintree[left],mintree[right]);
}
int min_query(int node,int b,int e,int l, int r){
    if(l>e || r<b)return highest(int);
    if(b>=l && e<=r){
        return mintree[node];
    }
    int left=2*node;
    int right=2*node+1;
    int mid=(b+e)/2;
    int x=min_query(left,b,mid,l,r);
    int y=min_query(right,mid+1,e,l,r);
    return min(x,y);
}
int max_query(int node,int b, int e, int l, int r){
    if(l>e || r<b)return lowest(int);
    if(b>=l && e<=r){
        return maxtree[node];
    }
    int left=2*node;
    int right=2*node+1;
    int mid=(b+e)/2;
    int x=max_query(left,b,mid,l,r);
    int y=max_query(right,mid+1,e,l,r);
    return max(x,y);
}
void init(){
    ms(arr);
...
void solution(){
    ...
    for(int i=1;i<=n-d+1;i++){
        int mini=min_query(1,1,n,i,i+d-1);
        int maxi=max_query(1,1,n,i,i+d-1);
        debug(i,maxi,mini);
        if(mini!=highest(int) && maxi!=lowest(int))ans=max(ans,maxi-mini);
    }
    printf("Case %d: %d\n",++cs,ans);
}
signed main()
