...
void update(int l,int r,int u,int v,int pos,int val){
    ...
    int right=2*pos+2;
    int mid=(l+r)/2;
    update(l,mid,u,v,left,val);
    update(mid+1,r,u,v,right,val);
    stree[pos]=stree[left]+stree[right];
}
ll query(int l,int r,int u,int v,int pos){
    ...
    int right=2*pos+2;
    int mid=(l+r)/2;
    ll x=query(l,mid,u,v,left);
    ll y=query(mid+1,r,u,v,right);
    return x+y;
}
void solution(){
    ms(stree);
