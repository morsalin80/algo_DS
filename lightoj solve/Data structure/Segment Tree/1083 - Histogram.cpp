...
int arr[maxx];
pair<int,int>segt[3*maxx];
void build(int node,int low,int high)
{
    if(low==high) {segt[node]=make_pair(arr[low],low); return; }
    int left=2*node;
    int right=2*node+1;
    int mid=(low+high)/2;
    build(left,low,mid);
    build(right,mid+1,high);
    segt[node]=min(segt[left],segt[right]);
}
pair<int,int> query(int node,int low,int high,int qlow,int qhigh)
{
    if(low>qhigh || high<qlow) return pair<int,int>(inf,inf);
    else if(low>=qlow && high<=qhigh) return segt[node];
    int left=2*node;
    int right=2*node+1;
    int mid=(low+high)/2;
    pair<int,int> l=query(left,low,mid,qlow,qhigh);
    pair<int,int> r=query(right,mid+1,high,qlow,qhigh);
    return min(l,r);
}
int n;
ll area(int i,int j)
{
    if(j<i) return 0;
    pair<int,int> mini=query(1,1,n,i,j);
    ll a1=(j-i+1)*mini.fi;
    ll a2=max(area(i,mini.se-1),area(mini.se+1,j));
    return max(a1,a2);
}
int cs=0;
void solution(){
